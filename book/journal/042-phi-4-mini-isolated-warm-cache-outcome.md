...
**Setup**: Updated [runtime/phi4_mini_ane.swift](https://github.com/videlalvaro/ane-book/blob/main/runtime/phi4_mini_ane.swift) with `--warmup-calls N` and `--warmup-token-id ID`. Warmup runs with separate `MLState`s for layer shards, then resets attention/KV write masks so the real generation KV cache starts clean. Tested on the 4-layer fused manifest.
